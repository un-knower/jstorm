...
public class KafkaConsumer {
        ...
        loop: while (i < size && errors < size + 1) {
            ...
            try {
                ...
                try {
                   ...
                   //todo
                    System.out.println("findLeader error, broker:" + host.toString() + ", will change to next broker index:" + (i + 1) % size);
                    if (consumer != null) {
                        consumer.close();
                        consumer = null;
                    }
                ...
                System.out.println("Error communicating with Broker:" + host.toString() + ", find Leader for partition:" + partition);
            } finally {
                if (consumer != null) {
                    consumer.close();
                    consumer = null;
    ...
    public long getOffset(String topic, int partition, long startOffsetTime) {
        ...
        if (simpleConsumer == null) {
            ...
            System.out.println("Error consumer is null get offset from partition:" + partition);
            return -1;
        }
